...
if __name__ == "__main__":
    ...
    step_radius = 0.001
    probe_radius = 0.001  # probe radius >= step radius
    ...
    num_particles_per_goal = 1  # number of plans per goal
    pos_limits = [-10, 10]
    vel_limits = [-10, 10]

    w_coll =1e-2 #5e-3  # for tuning the obstacle cost
    w_smooth = 5e-6 #1e-7  # for tuning the GP cost: error = w_smooth * || Phi x(t) - x(1+1) ||^2
    ...
    #--------------------------------- MPOT Init ---------------------------------
    model = fcVAE(input_size=4,in_channels=1, latent_dim=2, beta= .1, kl_weight=0.0005)
    model_name = "/home/kartik/mpot/saved_model/mpot_occupancy_mse_beta_0-01_l4.pt"
    file_ = model_name.split('/')[-1].split('.')[-2]
    model = torch.load(model_name, weights_only=False).to('cuda')
    model.eval()
    ...
    data = {}
    data["state trajectory"] = optim_state.X_history[:opt_iters]
    ...
    planner_visualizer.animate_opt_iters_joint_space_state(
        ...
        video_filepath=f'{base_file_name+file_}-joint-space-opt-iters.mp4',
    ...
    planner_visualizer.animate_opt_iters_robots(
        ...
        video_filepath=f'{base_file_name+file_}-traj-opt-iters.mp4',
        n_frames=max((2, opt_iters // 5)),
        anim_time=5
    )

...
class BetaVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.decode (z).view(-1, self.input_size)
    ...
    def compute_loss(self, x, output, mu, logsigma):
        ...
        #                                               x.view(x.shape[0], -1))
        mse_loss = nn.functional.mse_loss(output.view(x.shape[0], -1),
                                                     x.view(x.shape[0], -1))
...
class fcVAE(BetaVAE):
    def __init__(self,
                 ...
                 image_dim: int = 28,
                 beta: int = 1):
        super().__init__(input_size, in_channels, latent_dim, kl_weight, image_dim, beta)
        ...
        self.linear_mu = nn.Linear(256, self.latent_dim)
        self.linear_logsigma = nn.Linear(256, self.latent_dim)
    ...
    def decode(self, x):
        ...
        # return self.activation(z).view(-1,  self.input_size)
        return z

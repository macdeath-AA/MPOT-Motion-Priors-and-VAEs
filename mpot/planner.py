...
from mpot.utils import trajectory
...
class MPOT(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            multi_goal_states is None
        ):  # NOTE: if there is no goal, we assume here is at least one goal
            self.num_goals = 1
        else:
        ...
        self.pos_scaler = MinMaxCenterScaler(
            dim_range=[0, self.dim],
            min=self.pos_limits[:, 0],
            max=self.pos_limits[:, 1],
        )
        ...
        self.vel_scaler = MinMaxCenterScaler(
            dim_range=[self.dim, self.state_dim],
            min=self.vel_limits[:, 0],
            max=self.vel_limits[:, 1],
        )
    ...
    def reset(
        ...
        assert (
            self.start_state.shape[-1] == self.state_dim
        ), "start_state dimension should be dim * 2"

        if multi_goal_states is not None:
            self.multi_goal_states = multi_goal_states.detach().clone()
        assert (
            self.multi_goal_states.shape[-1] == self.state_dim
        ), "multi_goal_states dimension should be dim * 2"
    ...
    def get_prior_dists(self, initial_particle_means: torch.Tensor = None):
        ...
        # if self.init_trajs.ndim == 3:
        #     self.flatten_trajs = self.init_trajs.flatten(0, 1)
        # elif self.init_trajs.ndim == 2:
        #     self.flatten_trajs = self.init_trajs  # already flattened
        # else:
        #     raise ValueError(
        #         f"Unexpected shape for init_trajs: {self.init_trajs.shape}"
        #     )
    ...
    def get_random_trajs(self) -> torch.Tensor:
        ...
        if self.results_file_path is None:
            raise AttributeError("wrong file path.")

        print(f"Generating random trajectories from file...")

        with open(self.results_file_path, "rb") as f:
            trajectories = pickle.load(f)

        if not isinstance(trajectories, torch.Tensor):
            trajectories = torch.tensor(trajectories, **self.tensor_args)
        else:
            trajectories = trajectories.clone().detach().to(**self.tensor_args)
        trajectories = trajectories.unsqueeze(0)
        self.traj_dim = trajectories.shape

        print(f"Shape of loaded trajectories: {trajectories.shape}")

        flattened = trajectories.flatten(0, 1)
        print(f"shape after flatten: {flattened.shape}")

        return trajectories.flatten(0, 1)
        # return trajectories

    def optimize(self) -> Tuple[torch.Tensor, SinkhornStepState, int]:

        # print(
        #     f"Shape of self.flatten_trajs before init_state: {self.flatten_trajs.shape}"
        # )
        ...
        while self.sinkhorn_step._continue(state, iteration):
            ...
            # print(f"Shape of trajs after step: {trajs.shape}")
